...
export default function TimeSheetPage(){
  ...
  const selectionOk=()=>{if(printPoint||printReceipts)return true;setMessage('Selecione fichas de ponto e/ou recibos para imprimir.');return false}
    ...
    <Card style={{margin:'14px 0',padding:16}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:18,flexWrap:'wrap'}}>
        ...
        <div className="row-actions" style={{gap:8,flexWrap:'wrap'}}>
          <Button variant="secondary" icon={<UsersRound size={16}/>} onClick={()=>setBatchOpen((value)=>!value)} disabled={busy}>Gerar para todos</Button>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:10,marginTop:14}}>
        ...
        <div style={{border:'1px solid var(--border)',borderRadius:8,padding:'12px 14px'}}><strong style={{display:'block',fontSize:12}}>Recibos de benefícios</strong><small style={{display:'block',marginTop:4,color:'var(--muted)'}}>Recibos mensais dos benefícios lançados, salvos junto da ficha na pasta do mês.</small></div>
      </div>
    </Card>

    {batchOpen&&<Card className="print-batch-card" style={{marginBottom:14,maxWidth:760,marginLeft:'auto'}}>
      <div className="card-header" style={{alignItems:'flex-start'}}><div><h2>Documentos em lote</h2><p>Escolha ficha de ponto, recibos ou ambos. Imprimir gera os PDFs e abre a caixa do Windows; reimprimir apenas envia novamente o mês selecionado, sem duplicar arquivos.</p></div></div>
      <div style={{display:'grid',gap:10,margin:'14px'}}>
        ...
        <label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer'}}><input type="checkbox" checked={printReceipts} onChange={(event)=>setPrintReceipts(event.target.checked)}/><span><strong>Recibos de benefícios</strong><br/><small>Vale café de R$ 180,00 e vale-alimentação de R$ 510,00 para cada colaborador, além de outros benefícios lançados.</small></span></label>
      </div>
      <div className="row-actions" style={{justifyContent:'flex-end',flexWrap:'wrap',padding:'0 14px 14px'}}><Button variant="secondary" onClick={()=>setBatchOpen(false)} disabled={busy}>Cancelar</Button><Button variant="secondary" icon={<FileDown size={15}/>} onClick={generateAll} disabled={busy}>Somente gerar</Button><Button variant="secondary" icon={<RotateCcw size={15}/>} onClick={reprintAll} disabled={busy||(!printPoint&&!printReceipts)}>Reimprimir</Button><Button icon={<Printer size={15}/>} onClick={printAll} disabled={busy||(!printPoint&&!printReceipts)}>Imprimir</Button></div>
    </Card>}
