...
export default function PayrollPage(){
  ...
  const confirm=async(quinzena:number)=>{await window.fluxoDre.folha.confirm({funcionario_id:Number(employee),competencia,quinzena,data:today(),forma_pagamento:'PIX'});setVersion(v=>v+1);setMessage(quinzena===1?'Pagamento confirmado. A ficha de ponto e os recibos mensais foram gerados.':`${quinzena}ª quinzena confirmada e registrada.`)}
  ...
  return <>
    ...
    <Card className="payroll-filter"><Field label="Funcionário" wide><select value={employee} onChange={event=>setEmployee(event.target.value)}><option value="">Selecione um funcionário...</option>{employees.data?.map((item:any)=><option key={item.id} value={item.id}>{item.nome} — {cargos.data?.find((cargo:any)=>cargo.id===item.cargo_id)?.nome||'Sem cargo'}</option>)}</select></Field><Field label="Competência"><input type="month" value={competencia} onChange={event=>setCompetencia(event.target.value)}/></Field></Card>
