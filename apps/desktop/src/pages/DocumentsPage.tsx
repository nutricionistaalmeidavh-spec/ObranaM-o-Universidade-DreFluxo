...
export default function DocumentsPage() {
  ...
  const [scannerTarget, setScannerTarget] = useState<{ documentId: number; name: string } | null>(null)
  ...
  const importDoc = async () => {
    ...
    if (result) { setUpload(false); await Promise.all([docs.reload(), files.reload()]) }
  }

  return <>
    <PageHeader title="Central de documentos" description="Explore, organize, visualize e digitalize documentos de obra, contratos, compras e RH em um único lugar." actions={<><Button variant="secondary" icon={<FolderOpen size={16}/>} onClick={() => window.fluxoDre.documentos.openFolder()}>Abrir pasta no Windows</Button>{view === 'registros' && <Button icon={<FilePlus size={16}/>} onClick={() => { setForm({ ...form, obra_id: work, frente_id: front }); setUpload(true) }}>Importar documento</Button>}</>}/>
    <div className="toolbar"><div className="toolbar-left"><Segmented value={view} onChange={(value) => setView(value as 'pastas' | 'registros')} options={[{ value: 'pastas', label: 'Pastas' }, { value: 'registros', label: 'Registros' }]}/></div></div>
    {view === 'pastas' ? <FileExplorer rootId="documents" rootLabel="Documentos" documentFeatures title="Pastas de documentos" description="Navegue, visualize, organize e digitalize arquivos dentro da área documental protegida do Fluxo DRE."/> : <>
