...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-shell ${collapsed ? 'sidebar-collapsed' : ''}`}>
    <aside className="sidebar">
      ...
      <nav>{groups.map((group) => <div className="nav-group" key={group.label}>{group.to?<NavLink to={group.to} className="nav-label nav-label-link" style={groupLinkStyle} title={collapsed?group.label:undefined}>{group.label}</NavLink>:<span className="nav-label">{group.label}</span>}{group.items.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} end={to === '/'} title={collapsed ? label : undefined}><Icon size={18}/><span>{label}</span></NavLink>)}</div>)}</nav>
