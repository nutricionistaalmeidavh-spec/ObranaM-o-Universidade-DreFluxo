...
function buildPrintBatchHtml(entries, selection) {
  ...
  return '<!doctype html><html><head><meta charset="utf-8"><style>'+[
    ...
    '.receipt-document .signature{width:45%;margin:25px 12px 10px auto;border-top:1px solid #111;text-align:center;padding-top:4px}'
...
class TimeService {
  ...
  receiptHtml(data,company,cargo,benefits,paymentDate) {
    ...
    const items=normalizeMhBenefits(benefits)
  ...
  async printEntries(entries,payload) {
    ...
    const html=buildPrintBatchHtml(successful,{point:payload.point,receipts:payload.receipts})
    const htmlPath=path.join(os.tmpdir(),'fluxo-dre-impressao-'+validCompetence(payload.competencia)+'-'+Date.now()+'.html')
    const win=new BrowserWindow({show:false,webPreferences:{sandbox:true,contextIsolation:true,nodeIntegration:false}})
    try {
      ...
      return {results:entries,employees:successful.length,documents:successful.length*(Number(Boolean(payload.point))+Number(Boolean(payload.receipts))),printed:Boolean(outcome.success),canceled}
    } finally {
      win.destroy()
...
module.exports={TimeService,monthDays,addMinutes,jitter,mergeGeneratedPdfs,normalizeMhBenefits,buildPrintBatchHtml}
