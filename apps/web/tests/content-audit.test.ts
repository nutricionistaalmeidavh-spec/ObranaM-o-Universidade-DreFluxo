...
describe('auditoria semântica do banco de questões', () => {
  ...
  it('incorpora banco adicional de Português sem alterar a quantidade por unidade', () => {
    ...
    expect(variants.filter(item => hasQuestionVisual(item.prompt)).length).toBeGreaterThanOrEqual(320);
  ...
  it('audita todas as variantes dos bancos adicionais', () => {
    ...
    expect(renderedVariants.filter(item => hasQuestionVisual(item.prompt)).length).toBeGreaterThanOrEqual(320);
